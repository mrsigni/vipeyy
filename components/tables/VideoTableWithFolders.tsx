...
interface DeleteQueueProgressProps {
...
//===== DeleteQueueProgress Component =====
const DeleteQueueProgress: React.FC<DeleteQueueProgressProps> = ({ queue, onClose, isVisible }) => {
  ...
  const getStatusIcon = (status: DeleteQueueItem["status"]) => {
    switch (status) {
      case "completed":
    ...
    <div className="fixed bottom-4 right-4 bg-white rounded-lg shadow-lg border border-gray-200 w-80 max-w-[calc(100vw-2rem)] z-50">
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-200">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-medium text-gray-900">Deleting Files</h3>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="p-4 border-b border-gray-200">
        <div className="flex items-center justify-between text-sm text-gray-600 mb-2">
          <span>
            {completedCount} of {totalCount} deleted
      ...
      {processingItem && (
        <div className="p-3 bg-red-50 border-t border-gray-200">
          <div className="text-sm text-red-800">
            <span className="font-medium">Deleting:</span> {processingItem.name}
          </div>
        </div>
      )}
    </div>
...
export default function VideoTableWithFolders() {
  ...
  const handleDeleteVideo = async (videoId: number) => {
    ...
    const deleteItem: DeleteQueueItem = {
      id: videoId.toString(),
    ...
    setDeleteQueue([deleteItem]);
    setShowDeleteQueue(true);

    await processDelete(deleteItem);
  ...
  const handleBulkDelete = async () => {
    try {
      ...
      if (failedItems.length === 0) {
        ...
        toast.error(`Deleted ${completedCount - failedItems.length} items, ${failedItems.length} failed`);
      }
      ...
      // Auto-hide queue after completion
      setTimeout(() => {
        setShowDeleteQueue(false);
        setDeleteQueue([]);
      }, 3000);
