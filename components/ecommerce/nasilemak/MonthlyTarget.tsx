      ...
      <div className="px-5 pt-5 bg-white shadow-default rounded-t-2xl pb-11 dark:bg-gray-900 sm:px-6 sm:pt-6">
        <div className="mb-4">
          <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">
            Target Pendapatan Harian (Admin)
          </h3>
          <p className="mt-1 font-normal text-gray-500 text-theme-sm dark:text-gray-400">
            Pantau progres penghasilan platform hari ini terhadap target harian sebesar ${target}.
          </p>
        </div>
        <div className="relative">
          ...
          <span className="absolute left-1/2 top-full -translate-x-1/2 -translate-y-[95%] rounded-full bg-success-50 px-3 py-1 text-xs font-medium text-success-600 dark:bg-success-500/15 dark:text-success-500">
            {progress >= 100 ? "Target Tercapai" : `+${(100 - progress).toFixed(0)}% lagi`}
          </span>
        </div>
        <p className="mx-auto mt-10 w-full max-w-[380px] text-center text-sm text-gray-500 sm:text-base">
          Platform telah mendapatkan total {views} views dan penghasilan sebesar ${earnings.toFixed(2)} hari ini.
        </p>
      </div>
